...
def load_dataset(dataset_name, balance=False, discretize=True, dataset_folder='./'):
    ...
    if dataset_name == 'adult':
        ...
        dataset = load_csv_dataset(
            os.path.join(dataset_folder,
                         'lendingclub/LoanStats3a_securev1.csv'),
...
def load_csv_dataset(data, target_idx, delimiter=',',
    ...
    Args: 
        ...
        categorical_features: list of ints, indices of categorical features, if None, we infer from the data
        features_to_use: list of ints, indices of features to use, if None all are used, except target_idx
    ...
    if features_to_use is not None:
        ...
        if categorical_features is not None:
            categorical_features = [features_to_use.index(
        ...
        if categorical_features:
            categorical_features = [
                x if x < target_idx else x - 1 for x in categorical_features]

    if categorical_features is None:
        categorical_features = [f for f in range(
    ...
    if balance:
        ...
        idxs = np.concatenate([np.random.choice(np.where(labels == label)[0], min_labels) for label in np.unique(labels)])
        data = data[idxs]
        labels = labels[idxs]
        ret.data = data
        ret.labels = labels
